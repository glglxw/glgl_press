...
<template>
  ...
  <div 
    v-else
    class="w-[794px] h-[1123px] flex-shrink-0 relative overflow-hidden shadow-2xl origin-top font-serif"
  ...
  >
    ...
    <!-- Header -->
    <header class="pt-12 pb-6 px-12 flex flex-col items-center border-b-4 border-double relative z-10" :style="{ borderColor: duskvolColors.accent }">
      <div class="w-full flex justify-between items-end mb-2 font-serif text-xs tracking-[0.2em] opacity-60">
        <span 
          class="cursor-pointer"
          :class="{ 'ring-2 ring-blue-500': isSelected('date') }"
          @click="(e) => handleSelect('date', 'Date', e)"
        >{{ data.date }}</span>
        <span>VOL. {{ Math.floor(Math.random() * 1000) }} • PRICE: 1 COIN</span>
      </div>
      <h1 class="text-6xl font-serif font-bold tracking-tighter uppercase mb-2 flex items-center gap-4" :style="{ textShadow: `0 0 10px ${duskvolColors.accent}` }">
        <Ghost class="w-10 h-10 opacity-80" />
        The Duskvol Chronicle
      </h1>
      <div class="w-full h-px bg-current opacity-30"></div>
      <div class="text-sm font-serif italic mt-1 opacity-80">"Whispers from the Void, Truths from the Shadows"</div>
    </header>

    <!-- Page 1 -->
    <div v-if="page === 1" class="flex-1 px-12 py-8 grid grid-cols-3 gap-8 relative z-10">
      <!-- Left Column -->
    ...
    <!-- Page 2 -->
    <div v-if="page === 2" class="flex-1 px-12 py-8 grid grid-cols-12 gap-8 relative z-10">
    ...
    <!-- Footer -->
    <div class="h-16 flex items-center justify-center border-t-4 border-double mx-12 mb-8" :style="{ borderColor: duskvolColors.border }">
